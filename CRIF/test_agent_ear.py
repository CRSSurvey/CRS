...
def agent_eval(agent, load_model_type, dm, write_fp):
    use_gpu = True
    env = dm
    ...
    for e_data in tqdm(random.sample(ui_test_list, test_size), ncols=0):
        ...
        # metric
        feature = dm.config.config.item_info[item]
        with open(write_fp, 'a') as f:
            f.write('tarting new, episode_num: {}\n user ID: {}, item ID: {} episode count: {}, feature: {}\n'.format(episode_num, user, item, episode_num, feature))
        reward, turn, success, state_list, rec_result = agent.PG_eva_one_episode(user, item)
        if success:
            with open(write_fp, 'a') as f:
                f.write(f"success: {success}, turn: {turn}, rec list: {rec_result[0]}, gt: {rec_result[1]}\n")
            result_dic[turn] = result_dic.get(turn,[])+([rec_result])
        f.close()
        # metric end
        if 4 >= turn >= 2:
        ...
        if success:
            ...
            Rank += (1 / math.log(t + 3, 2) + (1 / math.log(t + 2, 2) - 1 / math.log(t + 3, 2)) / math.log(True + 1, 2)) # (1 / math.log(t + 2, 2)
    # metric
    with open(f"./data/{args.data_name}/interaction-log/interact_log.pickle", "wb") as fp:   #Pickling
...
if args.data_name == LAST_FM:
    ...
    model_name = "iter299-2022-10-06" # lastfm
elif args.data_name == YELP:
    config = ConfigYelp()
    user_att_count_not_subset.set_config(config)
    date_str = datetime.date.today().isoformat()
    # model_name = f"iter800-{date_str}" # yelp 2loss
    # model_name = "iter800-2022-09-06"
    model_name = "iter800-2022-10-26"
...
# metric
write_fp = './data/{}/interaction-log/interact_log.txt'.format(args.data_name)
agent_eval(agent, "PG", dm, write_fp)
# metric end
