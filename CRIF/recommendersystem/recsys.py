...
class recsys():
    def __init__(self, config, convhis=None, use_gpu=None):
        ...
        self.attribute_parent_mat = np.zeros([self.parent_attribute_num, self.attribute_num]) # 33*33
    ...
    # edge_index: adj_index[0]:[u1,u+i1,u2,u+i2,u+i1,att1+u+i,u+i2,att2+u+i]
    # adj_index[1]: [u+i1,u1,u+i2,u2,att1+u+i,u+i1,att2+u+i,u+i2]
    def item_one_step_train_update(self, user_list, edge_index, known_pos_att_list, known_pos_att_mask,
                            known_neg_att_list, known_neg_att_mask, pos_item_list, neg_item_list1,
                            neg_item_mask1, known_neg_item_list, known_neg_item_mask):

        self.rec.graph_prop(edge_index)

        self.rec.update_neg_user_embedding(user_list, pos_item_list, known_neg_item_list, known_neg_item_mask)
        item_feature = self.rec.graph_rep[self.rec.item_graph_index[(pos_item_list.unsqueeze(-1)).type(torch.long)]]
        if self.rec.neg_user.size()[0] != item_feature.size()[0]:
            # print (self.rec.neg_user.size(), item_feature.size())
            return 0
