...
def rank_items(given_preference, userID, itemID, skip_big_feature, FM_model, candidate_list, write_fp, after_update):
    ...
    FM_model.eval()
    start = time.time()
    mini_ui_pair = np.zeros((len(candidate_list), 2)) # (len(candidate_list),2)
    ...
    if len(y_true) > 1:
        ...
        if  target_position < 1:
            recall_1=1.0
            MRR_1=1/(target_position+1)
            recall_5=1.0
            MRR_5=1/(target_position+1)
            recall_10=1.0
            MRR_10=1/(target_position+1)
    ...
    return ranked_item,recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10
