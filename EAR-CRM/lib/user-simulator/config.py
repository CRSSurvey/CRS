...
class _Config():
    ...
    def init_basic(self):
        ...
        with open('../../data/{}/FM_valid_list.pickle'.format(dir), 'rb') as f:
            self.valid_list = pickle.load(f) # 15471
        with open('../../data/{}/FM_test_list.pickle'.format(dir), 'rb') as f:
            self.test_list = pickle.load(f) # 8312
    ...
    def change_param(self, playby, eval, update_count, update_reg, purpose, mod, mask, interact):
        self.play_by = playby
        self.eval = eval
        ...
        if self.mod == 'crm' or self.play_by == 'RO' or self.interact == 0:
            category_max = 0
            feature_max = 0
            for k, v in self.item_dict.items():
                if max(v['categories']) > category_max:
