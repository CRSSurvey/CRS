...
def main():
    ...
    parser.add_argument('-inputdim', type=int, dest='inputdim', help='input dimension', default = 89)
    parser.add_argument('-hiddendim', type=int, dest='hiddendim', help='hidden dimension', default = 64)
    parser.add_argument('-outputdim', type=int, dest='outputdim', help='output dimension', default = 34)
    ...
    parser.add_argument('-rec', type=str, dest='rec', help='rec model', default = 'FM')
    parser.add_argument('-interact', type=int, dest='interact', help='0 for without preferred attr info', default = 1)


    A = parser.parse_args()
    print('Arguments loaded!')
    if A.mod == 'ear':
        inputdim = 126#89 #TODO: all state
    else:
        inputdim = 126#33 #TODO: all state

    PN = PolicyNetwork(input_dim=inputdim, dim1=A.hiddendim, output_dim=A.outputdim)
    ...
    for epoch in range(8): # CART 是10
        ...
        if A.rec =='FM':
            if A.interact == 0:
                model_name = '../../../data/PN-model-{}/pretrain-model-interact0.pt'.format(A.mod)
            else:    
                model_name = '../../../data/PN-model-{}/pretrain-model.pt'.format(A.mod)
        if A.rec =='transE':
            model_name = '../../../data/PN-model-{}-{}/pretrain-model.pt'.format(A.mod, A.rec)
